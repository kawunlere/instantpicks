...
function generatePicks(data) {
  const { formA, formB, posA, posB } = data;
  ...
  let pHome = Math.max(0.1, Math.min(0.7, baseHome + formDelta + posDelta));
  let pAway = Math.max(0.1, Math.min(0.7, baseAway - formDelta - posDelta));
  let pDraw = Math.max(0.15, Math.min(0.45, baseDraw + (1 - Math.abs(formPctA - formPctB)) * 0.15));
  ...
  // Generate 5-7 dynamic picks based on the data
  const picks = [];
  
  // Always show main result
  if (aStrong && !bStrong) {
    picks.push({ pick: "Home Win (1)", conf: Math.round(pHome * 100), risk: "medium", why: `${data.teamA} in strong form (${scoreA}/15 pts)` });
  } else if (bStrong && !aStrong) {
    picks.push({ pick: "Away Win (2)", conf: Math.round(pAway * 100), risk: "medium", why: `${data.teamB} in strong form (${scoreB}/15 pts)` });
  } else if (balanced) {
    picks.push({ pick: "Double Chance (1X or X2)", conf: Math.round((1 - Math.max(pHome, pAway)) * 100), risk: "low", why: "Both teams evenly matched" });
  } else if (aStrong) {
    picks.push({ pick: "Home Win or Draw (1X)", conf: Math.round((pHome + pDraw) * 100), risk: "low", why: `${data.teamA} favored but safe play` });
  } else {
    picks.push({ pick: "Away Win or Draw (X2)", conf: Math.round((pAway + pDraw) * 100), risk: "low", why: `${data.teamB} slight favorite` });
  }
  
  // Goals picks - vary based on form
  if (aStrong && bStrong) {
    // Both strong = high scoring likely
    picks.push({ pick: "Over 1.5 Goals", conf: 75, risk: "low", why: "Both teams attacking" });
    picks.push({ pick: "Over 2.5 Goals", conf: 62, risk: "medium", why: "Strong attacks both sides" });
    picks.push({ pick: "BTTS: Yes", conf: 65, risk: "medium", why: "Both teams scoring" });
  } else if (aWeak && bWeak) {
    // Both weak = low scoring likely
    picks.push({ pick: "Under 2.5 Goals", conf: 70, risk: "low", why: "Weak attacks both sides" });
    picks.push({ pick: "Under 1.5 Goals", conf: 55, risk: "medium", why: "Possible goalless" });
    picks.push({ pick: "BTTS: No", conf: 60, risk: "medium", why: "Struggles to score" });
  } else {
    // Mixed
    picks.push({ pick: "Over 1.5 Goals", conf: 70, risk: "low", why: "Likely at least 2 goals" });
    picks.push({ pick: "BTTS: Yes", conf: 58, risk: "medium", why: "Both may find net" });
    picks.push({ pick: "Over 2.5 Goals", conf: 52, risk: "medium", why: "Possible high scoring" });
  }
  
  // Position-based pick
  if (posA <= 3 && posB >= 10) {
    picks.push({ pick: "Home Win (1)", conf: Math.min(85, Math.round(pHome * 100) + 10), risk: "low", why: `${data.teamA} top 3 vs ${data.teamB} bottom half` });
  } else if (posB <= 3 && posA >= 10) {
    picks.push({ pick: "Away Win (2)", conf: Math.min(85, Math.round(pAway * 100) + 10), risk: "low", why: `${data.teamB} top 3 vs ${data.teamA} bottom half` });
  } else {
    picks.push({ pick: "Half-time: Draw", conf: 55, risk: "medium", why: "Tight first half likely" });
  }
  
  // Add 1-2 more based on situation
  if (pDraw > 0.32) {
    picks.push({ pick: "Draw (X)", conf: Math.round(pDraw * 100), risk: "high", why: "High draw probability" });
  }
...
async function apiAnalyze(request, env) {
  try {
    ...
    let picks = generatePicks(data);
    
    // AI enhancement if conversation provided
    if (conversation && env.GEMINI_API_KEY) {
      const prompt = `Match: ${teamA} (form ${data.formA.join(',')}, pos ${data.posA}) vs ${teamB} (form ${data.formB.join(',')}, pos ${data.posB})
Current picks: ${picks.map(p => `${p.pick} (${p.conf}%)`).join(', ')}
User observation: "${conversation}"

Adjust confidence values based on user observation. Reply EXACTLY 5 lines:
PICK1: [same name]|[new conf 40-85]|[updated why]
PICK2: [same name]|[new conf 40-85]|[updated why]
PICK3: [same name]|[new conf 40-85]|[updated why]
PICK4: [same name]|[new conf 40-85]|[updated why]
PICK5: [same name]|[new conf 40-85]|[updated why]`;
    ...
    return new Response(JSON.stringify({ ok: true, predId, picks }), { headers: { "Content-Type": "application/json" } });
  } catch (e) {
  ...
  if (env.PICKS_KV) {
    ...
    if (predId) {
      ...
      if (predRaw) {
        ...
        const key = `${pred.platform}_form${sA-sB}_pos${pred.posB-pred.posA}`;
...
<div class="card glow">
  <h2 class="ct">WELCOME</h2>
  <p class="txt">Your smart betting co-pilot. Quick analysis, smart picks, real learning from every game.</p>
  <a href="/analyze" class="btn">START ANALYZING</a>
</div>
<div class="card">
  <h3 class="ct">QUICK TIPS</h3>
  <div class="muted">• Tap W/L/D buttons to enter form (fast)</div>
  <div class="muted">• Describe the game for better picks</div>
  <div class="muted">• Log WIN/LOSE to help the system learn</div>
  <div class="muted">• Bet only what you can afford to lose</div>
</div>
<div class="card warn">
  <b class="warn-text">⚠ DISCIPLINE FIRST</b>
  <p class="muted" style="margin-top:8px">No pick is 100% guaranteed. Bet within your means.</p>
</div>`,

  analyze: `${nav("analyze")}
<div class="head"><div class="logo">⚡ ANALYZE ⚡</div><div class="tag">SMART MATCH ANALYSIS</div></div>
<div class="card">
  <h3 class="ct">MATCH</h3>
  <label>PLATFORM</label>
  <select id="platform">${PLATFORMS.virtual.map(p => `<option value="${p.id}">${p.name} — ${p.game}</option>`).join("")}</select>
  <div class="row">
    <div><label>TEAM A</label><input id="team_a" placeholder="e.g. Chelsea"></div>
    <div><label>TEAM B</label><input id="team_b" placeholder="e.g. Tottenham"></div>
  </div>
  <div class="row">
    <div><label>TEAM A POS</label><input id="pos_a" type="number" placeholder="3"></div>
    <div><label>TEAM B POS</label><input id="pos_b" type="number" placeholder="7"></div>
  </div>
</div>
<div class="card">
  <h3 class="ct">TEAM A FORM (LAST 5)</h3>
  <div class="qt-row">
    <button type="button" class="qt" data-target="form_a" data-v="W">WIN</button>
    <button type="button" class="qt" data-target="form_a" data-v="D">DRAW</button>
    <button type="button" class="qt" data-target="form_a" data-v="L">LOSS</button>
  </div>
  <input id="form_a" placeholder="W,L,D,W,W" required>
  <button type="button" class="clear-btn" data-target="form_a">CLEAR</button>
</div>
<div class="card">
  <h3 class="ct">TEAM B FORM (LAST 5)</h3>
  <div class="qt-row">
    <button type="button" class="qt" data-target="form_b" data-v="W">WIN</button>
    <button type="button" class="qt" data-target="form_b" data-v="D">DRAW</button>
    <button type="button" class="qt" data-target="form_b" data-v="L">LOSS</button>
  </div>
  <input id="form_b" placeholder="L,W,L,D,W" required>
  <button type="button" class="clear-btn" data-target="form_b">CLEAR</button>
</div>
<div class="card">
  <h3 class="ct">HOW IS THE GAME PLAYING? (OPTIONAL)</h3>
  <textarea id="conv" rows="3" placeholder="e.g. Team A pressing high, Team B defensive, no goals yet..."></textarea>
  <button class="btn" id="analyzeBtn">⚡ ANALYZE ⚡</button>
</div>
<div id="result"></div>`,

  ask: `${nav("ask")}
<div class="head"><div class="logo">⚡ ASK ⚡</div><div class="tag">SMART BETTING QUESTIONS</div></div>
<div class="card glow">
  <h3 class="ct">INSTANT PICKS ASSISTANT</h3>
  <p class="muted">Ask about picks, patterns, strategy, bankroll management.</p>
</div>
<div class="card">
...
<div class="card glow"><h3 class="ct">PATTERN ENGINE</h3><p class="muted">Learning from your results.</p></div>
...
<div class="card"><h3 class="ct">VALUE BETTING</h3><p class="txt">Odds higher than true probability = value bet.</p></div>
<div class="card"><h3 class="ct">BANKROLL</h3><p class="txt">Max 2-5% per bet. Never chase losses.</p></div>`,

  about: `${nav("about")}
<div class="head"><div class="logo">⚡ ABOUT ⚡</div></div>
<div class="card glow"><h2 class="ct">MISSION</h2><p class="txt">Help people make smarter betting decisions.</p></div>
...
<div id="panel" style="display:none"><div class="card"><h3 class="ct">STATS</h3><div id="a-stats" class="muted">Loading...</div></div></div>`
};
...
.clear-btn{background:rgba(255,51,51,.1);color:var(--red);border:1px solid var(--red);border-radius:5px;padding:4px 10px;font-size:10px;cursor:pointer;font-family:inherit;margin-top:5px;display:inline-block}
...
document.querySelectorAll('.clear-btn').forEach(b=>{
  b.addEventListener('click',()=>{
    document.getElementById(b.dataset.target).value='';
  });
});
...
document.getElementById('analyzeBtn')?.addEventListener('click',async()=>{
  ...
  if(d.ok){
    ...
    res.innerHTML='<div class="head"><div style="font-size:18px;font-weight:800">'+d.picks[0].match||'MATCH'+' <span class="hl">VS</span></div></div><div id="picksList"></div><a href="/analyze" class="btn">NEW ANALYSIS</a>';
    const list=document.getElementById('picksList');
    d.picks.forEach((p,i)=>{
      const card=document.createElement('div');
      card.className='pick-card risk-'+p.risk;
      card.innerHTML='<div class="pick-header"><span class="tag2">PICK #'+(i+1)+'</span><span class="tag-risk risk-'+p.risk+'">'+p.risk.toUpperCase()+'</span></div><div class="pick-name">'+p.pick+'</div><div class="pick-conf">'+p.conf+'%</div><div class="pick-why">'+p.why+'</div><div class="pick-btns"><form><input type="hidden" name="pid" value="'+d.predId+'"><input type="hidden" name="p" value="'+p.pick+'"><button type="submit" name="o" value="win" class="btn-win">✓ WIN</button><button type="submit" name="o" value="lose" class="btn-lose">✗ LOSE</button></form></div>';
      list.appendChild(card);
    });
    list.querySelectorAll('form').forEach(f=>f.addEventListener('submit',async e=>{
      e.preventDefault();
      const fd=new FormData();
      ...
      alert('Logged! Thanks for teaching the system.');
      loadStats();
    }));
...
async function loadPatterns(){
  ...
  if(!active.length){list.innerHTML='<span>Need 10+ results to detect patterns</span>';return}
  list.innerHTML=active.slice(0,10).map(([key,r])=>{
    return '<div style="background:rgba(0,255,136,.05);border-left:3px solid var(--green);padding:10px;margin:6px 0;border-radius:6px"><b class="hl">'+key.replace(/_/g,' ')+'</b><br><span class="muted">'+r.hits+'/'+r.total+' = '+r.confidence+'%</span></div>';
  }).join('');
  }catch(e){}}
loadPatterns();
...
async function adminLogin(){
  ...
  const d=await r.json();
  if(d.ok){document.getElementById('login-card').style.display='none';document.getElementById('panel').style.display='block';
